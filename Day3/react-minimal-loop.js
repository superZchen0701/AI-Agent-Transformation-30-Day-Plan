/**
 * 1、手写一个极简 ReAct 循环（不依赖任何框架）
 * 2、实现 ReAct 循环：Thought→Action→Observation 三段循环
 * 3、测试场景：简单数学计算：
 *    - 接一个真实工具（如 Day2/tool-registry.js 的 calculator），让模型在循环里自己决定是否调用
 *    - 用"3*7+12 等于多少"测试，确认它会循环【Thought→Action→Observation】→最终答案
 * 验收：终端能看到 Thought/Action/Observation 逐步打印
 */
...
2. 拿到工具返回的 Observation 后，才能输出 Finish[最终答案]。
...
class ReActAgent {
  ...
  async run(question) {
    ...
    while (currentStep < this.maxSteps) {
      ...
      if (!action) {
        throw new Error('警告:未能解析出有效的Action，流程终止。');
      }
      // 4 - 执行 Action
...
async function main() {
  ...
  // 运行Agent（用大数乘法，小模型易算错，迫使 LLM 调用工具验证）
  await agent.run('1234 * 5678 + 987 等于多少');
}
